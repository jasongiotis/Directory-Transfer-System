...
{  int port, sock;unsigned int serverlen;
   ...
   if ((rem = gethostbyname(ip.c_str())) == NULL) { /* Find server address */
   ...
   printf("Requested connection to host %s port %d\n", argv[2], port);
       ...
       printf("directory :%s\n",foldername.c_str());
      if (write(sock,foldername.c_str(), foldername.length()) < 0) {           //sending the requested directory to the client
         perror("write"); exit(1); }
      ...
      if (read(sock, &buffer_size, sizeof(int)) < 0) {        //recieving buffer size from the client
         perror("read"); exit(1); }
      
      
      if (read(sock, &x, sizeof(int)) < 0) {         /* recieving the number of files the server will send us*/
         perror("read"); exit(1); }
      printf("files to be received : %d \n",x);
      for(int i=0;i<x;i++){ //then for each file we take the data
         int f_size;
         char buf[buffer_size];
         bzero(buf,sizeof(buf));
         if(read(sock, buf, sizeof(buf))<0){perror("read");exit(1);}  //getting file name from server
         if(read(sock, &f_size, sizeof(int))<0){perror("read");exit(1);} // getting file size from server
      ...
      std::string dir=file_name;
         ...
         fs::create_directories(file_name);}  //we create the directories that contain the file if they dont exist
      ...
      if(f_size<=buffer_size){loops=1;}else{loops++;}
