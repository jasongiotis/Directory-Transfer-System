...
typedef struct service_args{ //arguments for the communication threads
   int buf_size;
   int newsock;  //socket id 

}service_args;

typedef struct job{  //our queue will contain job objects
...
void* service(void* arg){
   ...
   if(socket_map.find(newsock)==socket_map.end()){ //creating a mutex for each socket if
   ...
   char buf[buffer_size];
            ...
            if(!is_regular_file(file.status()))
               ...
               if(service_queue.size()==max_allowed_size){ pthread_cond_wait(&full_queue,&queue_usage);}
                      job s={
        ...
        s=j.sock;
         ...
         pthread_mutex_lock(socket_map[s]);
          ...
          strcpy(name,j.name.c_str());
             ...
             if(write(s,name,sizeof(name))<0){perror("write");exit(1);}
             if( write(s,&file_size,sizeof(file_size))<0){perror("write");exit(1);}
            ...
            file.open(name);
             ...
             for (int i = 0; i < lps; i++)
             {
               ...
               file.read(buff,sizeof(buff));
                if(write(s,buff,sizeof(buff))<0){perror("write");}
             }
             file.close();
            //  close(fd);
             socket_services[s]++;
...
{  int port, sock, newsock; 
   ...
   printf("Listening for connections to port %d\n", port);
    ...
    pthread_cond_init(&full_queue, NULL);
     ...
     for (int i=0;i<workers;i++){
          if(pthread_create(&worker_ths[i], NULL, &worker,  NULL)<0) {          /* Create child for serving the client */
            perror("pthread"); exit(1); /* Child process */
   ...
   while(1) {
     ...
     if(pthread_create(&th, NULL, &service, &s)<0) {          /* Create child for serving the client */
            perror("pthread"); exit(1); /* Child process */
